...
//            REPORT_DURATION_WARNING( { result = pthread_mutex_lock(&m_syncMutex); }, WarningReporting::TooLongWaitingForLock, _T("Aqcuiring CriticalSection took suspiciously long"));
            result = pthread_mutex_lock(&m_syncMutex);
            if (result != 0) {
